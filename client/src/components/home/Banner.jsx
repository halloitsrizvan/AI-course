...
function Banner() {
  ...
  return (
    <div className="relative max-w-7xl mx-auto my-10 p-4 md:p-8  rounded-xl overflow-hidden" style={{backgroundColor:"#DCF8FA"}} >
      ...
      >
        {SLIDES.map((slide, index) => (
          <div key={index} className="flex-shrink-0 w-full grid md:grid-cols-2 gap-8 items-center p-4">
            ...
            <div className="hidden md:flex justify-center order-1 md:order-2">
              <img 
                ...
                onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/400x450/4f46e5/ffffff?text=Image+Placeholder"; }}
              />
            </div>
          </div>
        ))}
      ...
      <button 
        onClick={prevSlide} 
        className="absolute top-1/2 left-4 transform -translate-y-1/2 bg-white p-3 rounded-full shadow-lg opacity-80 hover:opacity-100 transition duration-150 z-19 border border-gray-200"
      ...
      <button 
        onClick={nextSlide} 
        className="absolute top-1/2 right-4 transform -translate-y-1/2 bg-white p-3 rounded-full shadow-lg opacity-80 hover:opacity-100 transition duration-150 z-19 border border-gray-200"
